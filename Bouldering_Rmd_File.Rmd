...
As such, these will be main predictors used when it comes to making models in Section 2. 
...
The data set, *boulder_results.csv*, contains all of the necessary information and is available on both Github (where I will import it from) and Kaggle (see footnote 2).
```{r warning = FALSE}
...
Now, instead of looking at overall performance, let us consider how the distribution of tops and zones change between rounds. To do this, I will begin my creating a new table that includes indicators for which round a given climber's performance belongs. Following that, I will display the comparative distributions using a ridge plot.
```{r}
...
One final way of measuring each countries typical performance is efficiency, defined as *mean(total_tops)/*
...
his shows that North Macedonia (MKD), Taiwan (TPE), Nepal (NEP), and Macao (MAC) all are more efficient
...
With a few exceptions (Brazil, Ecuador, Guatemala, Pakistan, Uzbekistan, Mexico, Peru, South Africa, Estonia, and North Macedonia), the efficiency rates of most countries for both tops and zones is between 0.3 and 0.6. Notably, the countries that had the highet mean and median tops and zones are not also the most efficient countries. In fact, France, Austria, and Slovenia all landed around the middle for both efficiency plots. This does not mean that efficiency cannot be used in predictions but it does seem less helpful in terms of predicting whether a given climber actually has a reasonable chance of winning a competition.
...
The training and testing sets have been successfully created. Note that there are 19 potential predictors for the *Winner_Contender* column. 
...
```{r}
varImp(knn_fit2) #Qualification_Tops and Qualification_Zones are now the most important 
                 #features.
